...
#
# Usage: draw-types.py [-t <type>] [-m <max-levels>]
#                      [-x <exclude-type>] [-o <output-file>]
#                      [-i]
#                      <database>
#
# Schema of the DB
...
def draw_types(db, type_ids, max_levels, exclude_type_ids, show_id=False):
    ...
    while tasks:
        ...
        if _type.id not in has_labels:
            ...
            pass
        if _type.name in exclude_type_ids or ('@' + str(_type.id)) in exclude_type_ids:
            continue
...
def main():
    ...
    parser.add_argument('-m', '--max-levels', type=int, help='maximum number of levels to draw')
    ...
    for type_name in args.type:
        ...
        if type_name[1:].startswith('@'):
            ...
            pass
        pass
    draw_types(db, type_ids, args.max_levels or 5, args.exclude_type or [], show_id=args.show_id)
